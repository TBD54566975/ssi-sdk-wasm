...
describe('SSI tests', () => {
  ...
  test('Sign submission payload 2', async () => {
    const holderDID = await SSI.createDIDKey();
    
    const cred = { 
      "id": holderDID.didDocument.id, 
      "firstName": "bob",
    }
    const credSubject = JSON.stringify(cred)
    const vc = await SSI.createVerifiableCredential(holderDID.didDocument.id, holderDID.privKeyJWK, credSubject)

    const privKeyJWKJson = JSON.parse(holderDID.privKeyJWK)
    const secret = await jose.importJWK(privKeyJWKJson)
    const alg = 'EdDSA'

    const submissionId = "bob-submission-id"
    const definitionId = "2eddf25f-f79f-4105-ac81-544c988f6d78"
    ...
    const jwt = await new jose.SignJWT(vp)
...
test('neals test case', async () => {
  ...
  const definitionId = "affa237e-8523-48e4-8886-2909ac6b1bca"

...
(async () => {
  if (typeof window === "undefined") {
    ...
    const wasmFile = fs2.readFileSync(wasmFilePath);
    loadWasm(wasmFile);
  } else {
    fetch(wasmFilePath).then((response) => response.arrayBuffer()).then((wasmFile) => loadWasm(wasmFile));
  }
})();
async function loadWasm(wasmFile) {
  ...
  go.run(wasmInstance);
  wasmExports = { makeDid: global.makeDid, resolveDID: global.resolveDID };
}
function makeDid() {
  if (!wasmExports) {
